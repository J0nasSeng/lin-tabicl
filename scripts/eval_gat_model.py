...
def build_parser() -> argparse.ArgumentParser:
	...
	parser.add_argument("checkpoint", type=Path, help="TabICL checkpoint (.ckpt) to evaluate")
...
def _evaluate_class_count(model, args: argparse.Namespace, num_classes: int, device: str) -> list[dict]:
	...
	while processed < args.num_datasets:
		...
		for index in range(int(batch[0].shape[0])):
			...
			predictions = {"tabicl": sample.y_pred_test.numpy().astype(int)}
			for name, factory in BASELINE_FACTORIES.items():
				...
				predictions[name] = baseline.predict(np.nan_to_num(x_test)).astype(int)
			...
			for model_name, prediction in predictions.items():
				for score_name, score_fn in SCORES.items():
					rows.append({
						...
						"model": model_name,
						"score": score_name,
						"value": float(score_fn(y_test, prediction)),
					})
			processed += 1
...
def _plot_results(rows: list[dict], output_dir: Path) -> None:
	...
	for num_classes, class_rows in sorted(by_classes.items()):
		for score_name in dict.fromkeys(row["score"] for row in class_rows):
			...
			ax.set(title=f"K={num_classes}: mean {score_name}", ylabel=score_name, ylim=(0, 1))
			...
			ax.set(title=f"K={num_classes}: TabICL vs baselines", xlabel="TabICL", ylabel="baseline", xlim=(0, 1), ylim=(0, 1))
			ax.grid(alpha=0.25)
			ax.legend()
			fig.tight_layout()
