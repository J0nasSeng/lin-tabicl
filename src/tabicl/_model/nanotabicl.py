...
def _safe_standardize_with_train_stats(x: torch.Tensor, n_train: int, eps: float = 1e-6) -> torch.Tensor:
    """Standardize using train-part statistics in float32 for numerical stability.

    Using float16 with very small eps can underflow to zero and trigger divide-by-zero
    for constant features. We compute stats in float32, clamp std to eps, then cast back.
    """

    x_fp32 = x.float()
    train_x = x_fp32[:, :n_train]
    mean = train_x.mean(dim=1, keepdim=True)
    std = train_x.std(dim=1, unbiased=False, keepdim=True).clamp_min(eps)
    return ((x_fp32 - mean) / std).to(dtype=x.dtype)
...
class NanoTabICLv2(nn.Module):
    ...
    def forward(
        ...
        **_: typing.Any,
        ...
        _ = d  # accepted for Trainer API compatibility

        # ----- Embedding: standardize -> repeated feature grouping -> x embedding -> add y embedding to train
        x = _safe_standardize_with_train_stats(x, n_train=n_train)
        ...
        pred = self.out_mlp(self.out_ln(emb))  # output MLP
...
class InducedTransformerBlock(TableAttnBase):
    ...
    def forward(self, q, kv=None, q_max_idx: typing.Optional[int] = None, kv_max_idx: typing.Optional[int] = None):
        ...
        emb = self.tfm2(q, kv, q_max_idx=q_max_idx)
        return emb
...
class TransformerBlock(nn.MultiheadAttention, TableAttnBase):
    ...
    def forward(self, q, kv=None, q_max_idx: typing.Optional[int] = None, kv_max_idx: typing.Optional[int] = None):
        ...
        # Keep normalization and attention prep in fp32 to reduce fp16 overflow/underflow issues.
        x = torch.nan_to_num(q.float(), nan=0.0, posinf=1e4, neginf=-1e4)
        q = self.ln_attn(x)
        if kv is None:
            kv = q
        else:
            kv = torch.nan_to_num(kv.float(), nan=0.0, posinf=1e4, neginf=-1e4)
            kv = self.ln_attn(kv)
        ...
        x = x + self.mlp(self.ln_mlp(x))  # we use pre-norm here and for the attention as well
        return x.to(dtype=input_dtype)

    def attn(self, q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        if k.size(-2) == 0 or q.size(-2) == 0:
            return torch.zeros(q.shape[0], q.shape[-2], self.embed_dim, device=q.device, dtype=q.dtype)
...
class QASSMax(nn.Module):  # query-aware scalable softmax for better context length scaling
    ...
    def forward(self, q: torch.Tensor, n: int) -> torch.Tensor:
        ...
        logn = torch.tensor(math.log(max(1, n)), device=q.device, dtype=torch.float32).view(1, 1)
        q_fp32 = q.float()
        base = self.base_mlp(logn).view(1, num_heads, 1, head_dim)
        mod = 1 + torch.tanh(self.query_mlp(q_fp32))
        out = base * mod * q_fp32
