...
class Trainer:
    ...
    def build_model(self):
        ...
        self.model_config = {
            ...
            "graph_num_graphs": self.config.graph_num_graphs or self.config.icl_num_blocks,
            "graph_v1_prob": self.config.graph_v1_prob,
            "graph_v2_prob": self.config.graph_v2_prob,
            "graph_prob": self.config.graph_prob,
            "learnable_residual": self.config.learnable_residual,
        ...
        if self.ddp:
            self.model = DDP(
                ...
                find_unused_parameters=self.config.icl_backend in GRAPH_BACKENDS,
            )
    ...
    def configure_prior(self):
        ...
        if self.config.prior_dir is None:
            ...
            dataset = PriorDataset(
                ...
                graph_backend=self.config.icl_backend in GRAPH_BACKENDS,
                graph_num_graphs=self.config.graph_num_graphs or self.config.icl_num_blocks,
                graph_min_train_neighbors=self.config.graph_min_train_neighbors,
                graph_max_train_neighbors=self.config.graph_max_train_neighbors,
                graph_cross_label_fraction=self.config.graph_cross_label_fraction,
            ...
            if self.config.icl_backend in GRAPH_BACKENDS:
                raise ValueError("Graph backends require on-the-fly prior generation with graph metadata")
    ...
    def run_micro_batch(self, micro_batch, micro_batch_idx, num_micro_batches):
        ...
        ----------
        micro_batch : tuple
            Five tensor fields, optionally followed by a graph metadata object.
        ...
        graph_set = micro_batch[5] if len(micro_batch) == 6 else None
        if self.config.icl_backend in GRAPH_BACKENDS and graph_set is None:
            raise ValueError("Graph backend requires graph metadata in every batch")
        ...
        with self.amp_ctx:
            if self.regression:
                ...
                pred = self.model(micro_X, y_train, model_d, graph_set=graph_set)
                ...
                pred = self.model(micro_X, y_train, model_d, graph_set=graph_set)  # (B, test_size, max_classes)
    ...
    def run_batch(self, batch):
        ...
        if len(batch) not in (5, 6):
            raise ValueError("Expected five batch fields, optionally followed by graph metadata")
        graph_set = batch[5] if len(batch) == 6 else None
        tensor_fields = [t.to_padded_tensor(padding=0.0) if t.is_nested else t for t in batch[:5]]

        # Split the batch into micro-batches along the first dimension
        batch_size = tensor_fields[0].shape[0]
        num_micro_batches = math.ceil(batch_size / self.config.micro_batch_size)
        micro_batches = [torch.split(t, self.config.micro_batch_size, dim=0) for t in tensor_fields]
        ...
        for idx, micro_batch in enumerate(micro_batches):
            try:
                start = idx * self.config.micro_batch_size
                end = min(start + self.config.micro_batch_size, batch_size)
                if graph_set is not None:
                    micro_batch = (*micro_batch, graph_set.slice(slice(start, end)))
