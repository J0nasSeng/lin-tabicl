...
def _remap_class_ids(y: torch.Tensor, max_classes: int) -> torch.Tensor:
    """Randomly remap local class IDs to global IDs in [0, max_classes-1].

    This reduces systematic preference for low label IDs across generated
    datasets by sampling a fresh injective mapping per dataset.
    """
    ...
    if max_classes >= num_unique:
        target_ids = torch.randperm(max_classes, device=y.device, dtype=torch.long)[:num_unique]
    else:
        # Fallback when local classes exceed max_classes: remap within local range.
        target_ids = torch.randperm(num_unique, device=y.device, dtype=torch.long)

    remapped = y.clone()
    for src, dst in zip(unique, target_ids):
        remapped[y == src] = dst

    return remapped


class NanoTabICLPrior:
    """Lightweight prior wrapper compatible with SCMPrior.generate_dataset()."""
    ...
    def __call__(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        tensors = rand_dataset_filtered(
            x_cat_sizes=x_cat_sizes,
            y_cat_sizes=y_cat_sizes,
            n_samples=self.seq_len,
            multiclass_type=self.multiclass_type,
            multiclass_jitter_scale=self.multiclass_jitter_scale,
        )
        ...
        if self.num_classes > 0:
            y = _remap_class_ids(y, max_classes=self.max_classes)

        return X.to(self.device), y.float().to(self.device)
...
def rand_dataset_plain(
    ...
    n_samples: int,
    multiclass_type: str = "quantile",
    multiclass_jitter_scale: float = 0.0,
    ...
    for node_idx in range(n_nodes):
        ...
        node_values[node_idx], out_features = rand_node_func(
            ...
            multiclass_jitter_scale=multiclass_jitter_scale,
        )
...
def rand_dataset_filtered(
    ...
    while True:
        ...
        if y_cat_sizes[0] > 0:
            # Classification branch: keep labels discrete and check significance
            # against a random-label baseline via permutation test.
            y_np = y.long().cpu().numpy()
            if len(np.unique(y_np)) != y_cat_sizes[0]:
                continue  # reject datasets where the number of unique classes doesn't match the specified num_classes
            ...
            if oob_proba is None:
                continue
            if oob_proba.ndim == 1:
                oob_proba = oob_proba[:, None]

            valid_mask = np.isfinite(oob_proba).all(axis=1) & (oob_proba.sum(axis=1) > 0)
            if valid_mask.sum() < 8:
                continue

            y_true = y_np[valid_mask]
            y_pred = oob_proba[valid_mask].argmax(axis=1)
            observed_score = _balanced_accuracy(y_true, y_pred)
            ...
            Y_np = y.float().cpu().numpy().reshape(-1, 1)
...
def _balanced_accuracy(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Compute balanced accuracy over classes present in y_true."""
    labels = np.unique(y_true)
    if labels.size == 0:
        return 0.0
    recalls = [(y_pred[y_true == label] == label).mean() for label in labels]
...
def rand_node_func(
    ...
    for name, cat_size in cat_sizes.items():
        ...
        x[:, start_idx:end_idx], out_features[name] = rand_converter(
            x[:, start_idx:end_idx],
            cat_size,
            mode=multiclass_type,
            jitter_scale=multiclass_jitter_scale,
            is_label=name.startswith("y_"),
        )
        start_idx = end_idx
...
def _quantile_assign_classes(values: torch.Tensor, cat_size: int, jitter_scale: float = 0.0) -> torch.Tensor:
    """Assign classes from 1D values using quantile boundaries, like Reg2Cls."""
    if cat_size < 2:
        return torch.zeros(values.shape[0], 1, device=values.device, dtype=torch.long)

    T = values.shape[0]
    sorted_values, _ = torch.sort(values)
    steps = torch.arange(1, cat_size, device=values.device, dtype=torch.float32)
    ideal_indices = steps * (T / cat_size)

    if jitter_scale > 0.0:
        max_jitter = (T / cat_size) * jitter_scale
        jitter = (torch.rand(cat_size - 1, device=values.device) * 2.0 - 1.0) * max_jitter
    ...
    classes = (values.unsqueeze(-1) > boundaries.unsqueeze(0)).sum(dim=1)
    return classes.long().unsqueeze(-1)
...
def rand_converter(
    ...
    if mode == "quantile" and is_label:
        # Use a random projection to 1D, then quantile boundaries as in Reg2Cls.
        projection = rand_weights(1, x.shape[1]).to(x.device)
        scores = (standardize(x) * projection).sum(dim=-1)
        x_idxs = _quantile_assign_classes(scores, cat_size=cat_size, jitter_scale=jitter_scale)
        x_disc = rand_points(cat_size, cat_size)[x_idxs.squeeze(-1)]
        x = x_disc
    else:
        # Legacy stochastic behavior for compatibility.
        legacy_mode = randchoice(
            ["neigh_id", "neigh_disc", "neigh_func", "neigh_int", "softmax_id", "softmax_disc", "softmax_int"]
        )

        if legacy_mode.startswith('softmax'):
        ...
        if legacy_mode.endswith('_disc'):  # transform x for further use in the graph
            x = x_disc
        elif legacy_mode.endswith('_func'):  # don't have this for softmax because x_disc is already random for softmax
            x = rand_func(x_disc, cat_size, only_cheap=True)
        elif legacy_mode.endswith('_int'):
...
if __name__ == '__main__':
    ...
    labels = torch.stack(labels, dim=0)  # (n_datasets, n_samples)

    # count number of appearances of each class in each dataset
    n_classes = labels.max(dim=-1).values + 1
    class_counts = torch.stack([(labels == i).sum(dim=-1) for i in range(labels.max() + 1)], dim=-1)  # (n_datasets, n_classes)
    print("Class counts per dataset:")
    print(class_counts)
    print("Class counts over all datasets:")
    print(class_counts.sum(dim=0))
