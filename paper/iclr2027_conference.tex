...
\textbf{Tabular Foundation Models.} Prior-Data Fitted Networks~\citep{muller2022transformers} introduced the idea of pre-training a transformer on datasets drawn from a synthetic prior so that it approximates Bayesian inference by in-context learning. TabPFN~\citep{hollmann2023tabpfn} instantiated this idea for tabular classification on small datasets, and its successor extended it to regression and to larger, more heterogeneous data~\citep{hollmann2025accurate}. Because performance depends strongly on the pre-training distribution, a line of work targets the prior itself, for instance by mixing several synthetic data-generating mechanisms~\citep{zhang2025mitra}. A second line targets scale and architecture: TabICL~\citep{qu2025tabicl} employs a two-stage column-then-row attention scheme aimed at larger datasets, TabICLv2~\citep{qu2026tabiclv2} additionally releases its prior alongside the weights, and LimiX~\citep{zhang2025limixunleashingstructureddatamodeling} treats structured data as a joint distribution over variables and missingness to cover several tabular tasks with one model. Despite these differences, the models above perform in-context learning through attention in which every sample can attend to every other sample, so the cost of a forward pass grows quadratically with the context. Existing remedies largely preserve this all-pairs interaction: inducing-point attention reduces set self-attention to linear cost in the number of elements~\citep{lee2019set}, while IO-aware kernels reduce memory traffic without changing the attention pattern~\citep{dao2022flashattention}. In language modeling, sparse and low-rank attention mechanisms relax the all-pairs assumption~\citep{zaheer2020bigbird, beltagy2020longformer, choromanski2021rethinking}, but their sparsity patterns are defined over token positions rather than over labeled samples. We instead make the interaction pattern an explicit, label-informed graph over samples, which allows the effect of sparsification to be studied directly. Independently of these developments, tuned tree ensembles remain strong baselines on medium-sized tabular data~\citep{grinsztajn2022why_do_tree_models}.

\textbf{Graph Neural Networks and Graph Foundation Models.} Message passing is the standard computational pattern for learning on graphs, unified by the MPNN framework~\citep{gilmer2017neural_msg_passing}. Graph convolutional networks aggregate over neighborhoods with fixed weights~\citep{kipf2017semi}, whereas graph attention networks weight neighbors by learned attention~\citep{velickovic2018graph}; GATv2 shows that the original formulation realizes only static attention and proposes a dynamic variant~\citep{brody2022how_attentive}. Since exact aggregation is expensive on large graphs, neighbor sampling is a common remedy~\citep{hamilton2017inductive}, and our bounded degree budget plays an analogous role. Stacking many message-passing layers is known to make node representations increasingly indistinguishable, which motivates normalization~\citep{zhao2020pairnorm} and stochastic edge removal~\citep{rong2020dropedge}; the latter is also the closest precedent for resampling several graphs within a single forward pass. Most of these architectures perform best when connected nodes tend to share labels, and their behavior degrades under heterophily, which has prompted dedicated designs~\citep{zhu2020beyond, pei2020geomgcn}; accordingly, we restrict our claims to the homophilic setting. Closest to our work, PRODIGY~\citep{huang2023prodigy} enables in-context learning over graphs by connecting prompt examples and queries in a prompt graph, and thereby conditions a graph model on labeled context at inference time. Our setting differs in the direction of transfer: rather than pre-training on observed graphs, we sample graphs as part of a tabular prior and treat an observed graph as a special case at test time. Standardized node classification benchmarks provide the evaluation protocols for this modality~\citep{hu2020open}.
